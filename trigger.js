...
got.post(`https://api.travis-ci.org/repo/orcinustools/repository/requests`, {
  headers: {
  ...
  body: JSON.stringify({
    request: {
      message: `Trigger build at omura commit: ${gitCommitHash}`,
      branch: 'source',
    },    
  }),
})
.then(() => {
  console.log("Triggered build of omura");
})
.catch((err) => {
  console.error(err);
